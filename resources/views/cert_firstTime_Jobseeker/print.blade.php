...
<head>
    ...
    <style>
        @page { size: A4; margin: 0 12mm 12mm 12mm; }
        ...
        .receipt { margin-top: 18px; font-size: 8px; }
        .contact { margin-top: auto; font-size: 12px; border-top: 2px solid #e6e6e6; padding-top: 8px; display: flex; justify-content: space-between; align-items: center; }
        .contact .left { font-size: 12px; }
        .contact .right { font-size: 12px; }
        @media print {
            body, html { width: 210mm; height: 297mm; }
            .page { padding: 0 12mm 12mm 12mm; }
        ...
        <div class="content">
            <p>
                This is to certify that Mr./Ms./Mrs. <strong
                    class="name">{{ strtoupper($cert->resident->full_name_first) }}</strong>,
                a resident of <strong>{{ $cert->purok }}</strong>, Barangay
                {{ ucfirst(strtolower($barangayDetails->barangay_name ?? $barangayDetails->barangay ?? '')) }}, for <strong>{{ $cert->residence_period_years ?? '___' }}</strong> years, 
                is a qualified availee of <strong>RA 11261</strong> (First-Time Jobseeker Act).
            </p>

            <p>
                I further certify that the holder/bearer was informed of his/her rights including duties and responsibilities
                accorded by RA 11261 through the Oath of Undertaking he/she has assigned and executed in the presence of our barangay official.
            </p>
        ...
        <div class="signature-row">
            <div class="applicant">
                <div style="margin-top:30px; padding-top:6px; text-decoration:underline">
                    {{ strtoupper($cert->resident->full_name_first) }}<br>
                </div>
