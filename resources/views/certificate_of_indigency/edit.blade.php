...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            <h1 class="text-2xl font-bold">Edit Certificate of Indigency</h1>
            <a href="{{ route('certificate_of_indigency.index') }}" class="btn btn-outline-primary">Back to List</a>
        </div>

        <form action="{{ route('certificate_of_indigency.update', $certificate->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mt-8 flex items-center justify-end">
                <button type="button" class="btn btn-outline-danger" onclick="window.location.href='{{ route('certificate_of_indigency.index') }}'">Cancel</button>
