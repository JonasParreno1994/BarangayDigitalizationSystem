...
<head>
    <meta charset="UTF-8">
    <title>Barangay Clearance Report</title>
    <style>
        body {
...
<body>
    ...
    <div class="report-title">Barangay Clearance Report</div>
    ...
    <div class="certificate-section">
        <h3 class="section-title">Barangay Clearance ({{ $reportData->count() }})</h3>
        ...
        <table>
            ...
            <tbody>
                @foreach($reportData as $index => $clearance)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $clearance->resident->last_name }}, {{ $clearance->resident->first_name }} {{ $clearance->resident->middle_name }}</td>
                    <td>{{ Str::limit($clearance->purpose, 25) }}</td>
                    <td>{{ $clearance->date_of_issuance ? $clearance->date_of_issuance->format('m/d/Y') : 'N/A' }}</td>
                    <td>{{ $clearance->cedula_number ?? 'N/A' }}</td>
                    <td>{{ $clearance->or_number ?? 'N/A' }}</td>
                    <td>₱{{ number_format($clearance->amount_paid ?? 0, 2) }}</td>
        ...
        <div class="no-data">
            ...
            <p>No Barangay Clearance records found for the selected date range.</p>
        </div>
        @endif
    </div>
