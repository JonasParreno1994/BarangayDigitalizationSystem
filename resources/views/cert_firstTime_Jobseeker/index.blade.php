        ...
        <div x-data="multipleTable">
            ...
            <div class="panel mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <form action="{{ route('cert_firstTime_Jobseeker.report') }}" method="GET" class="mt-4">
            ...
            <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
                <div x-show="open" x-transition x-transition.duration.300 class="panel my-8 w-full max-w-4xl overflow-hidden rounded-lg border-0 p-0">
                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                        <div class="text-lg font-bold">ISSUE CERTIFICATE FOR FIRST TIME JOBSEEKER</div>
    ...
    Alpine.data('multipleTable', () => ({
        ...
        init() {
            this.datatable = new simpleDatatables.DataTable('#clearanceTable', {
                ...
                data: {
                    headings: ['ID', 'Name', 'Puroks', 'Date of Issuance','Status','Actions'],
                    data: [
                        @foreach ($certs as $certJobseeker)
                            [
                                '{{ $certJobseeker->id }}',
                                '{{ $certJobseeker->resident->last_name }}, {{ $certJobseeker->resident->first_name }}',
                                '{{ $certJobseeker->purok }}',
                                '{{ $certJobseeker->date_of_issuance->format('m/d/Y') }}',
                                `<span class="badge ${getStatusBadgeClass('{{ $certJobseeker->status }}')}">
                                    {{ $certJobseeker->status }}
                                </span>`,
                                `<div class="flex space-x-2">
                                    <a href="{{ route('cert_firstTime_Jobseeker.show', $certJobseeker->id) }}" class="btn btn-sm btn-info">View</a>
                                    <a href="{{ route('cert_firstTime_Jobseeker.print', $certJobseeker->id) }}" target="_blank" class="btn btn-sm btn-success">Print</a>
                                    <form action="{{ route('cert_firstTime_Jobseeker.destroy', $certJobseeker->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
