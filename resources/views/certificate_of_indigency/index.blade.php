    ...
    <div class="animate__animated p-6" :class="[$store.app.animation]">
        <div x-data="multipleTable">
            <div class="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary text-2xl font-bold">
                ...
                <h1 class="ltr:mr-4 rtl:ml-3 text-center w-full">List of Certificate of Indigency</h1>
            </div>
            <div class="panel mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <form action="{{ route('certificate_of_indigency.report') }}" method="GET" class="mt-4">
            ...
            <div class="panel mt-6">
                <table id="indigencyTable" class="whitespace-nowrap"></table>
            </div>
        </div>
            ...
            <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
                <div x-show="open" x-transition x-transition.duration.300 class="panel my-8 w-full max-w-4xl overflow-hidden rounded-lg border-0 p-0">
                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                        <div class="text-lg font-bold">ISSUE CERTIFICATE OF INDIGENCY</div>
                    ...
                    <div class="p-5">
                       <form id="indigencyForm" action="{{ route('certificate_of_indigency.store') }}" method="POST">
                            @csrf
        ...
        <div class="fixed inset-0 z-[999] hidden overflow-y-auto bg-[black]/60" id="viewIndigencyModal">
            <div class="flex min-h-screen items-start justify-center px-4">
                <div class="panel my-8 w-full max-w-4xl overflow-hidden rounded-lg border-0 p-0">
                    ...
                    <div class="p-5" id="indigencyDetailsContent">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('multipleTable', () => ({
        datatable: null,
        init() {
            this.datatable = new simpleDatatables.DataTable('#indigencyTable', {
                data: {
                        ...
                        @foreach ($certificates as $certificate)
                            [
                                ...
                                `<div class="flex space-x-2">
                                    <a href="{{ route('certificate_of_indigency.show', $certificate->id) }}" class="btn btn-sm btn-info">View</a>
                                    <a href="{{ route('certificate_of_indigency.print', $certificate->id) }}" target="_blank" class="btn btn-sm btn-success">Print</a>
                                    <a href="{{ route('certificate_of_indigency.edit', $certificate->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                    <form action="{{ route('certificate_of_indigency.destroy', $certificate->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
...
function showIndigencyModal(certificateId) {
    document.getElementById('viewIndigencyModal').classList.remove('hidden');
    
    fetch(`/certificate_of_indigency/${certificateId}`)
        .then(response => response.text())
        .then(html => {
            document.getElementById('indigencyDetailsContent').innerHTML = html;
        })
        .catch(error => {
            console.error('Error loading certificate details:', error);
            document.getElementById('indigencyDetailsContent').innerHTML = `
...
function printIndigency(certificateId) {
    const printWindow = window.open(
        `/certificate_of_indigency/${certificateId}/print`, 
        '_blank',
...
function closeViewModal() {
    document.getElementById('viewIndigencyModal').classList.add('hidden');
}

// Close modal when clicking outside
document.getElementById('viewIndigencyModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closeViewModal();
    }
});
