        ...
        <div x-data="multipleTable">
            ...
            <div class="panel mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <form action="{{ route('certificate_of_death.report') }}" method="GET" class="mt-4">
                        ...
                        <form id="deathCertificateForm" action="{{ route('certificate-of-death.store') }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-input" name="issued_by" required placeholder="Issued By">
                                </div>
                            </div>
    ...
    Alpine.data('multipleTable', () => ({
        ...
        init() {
            this.datatable = new simpleDatatables.DataTable('#deathCertificateTable', {
                data: {
                    headings: ['ID', 'Resident', 'Date of Death', 'Date Issued', 'Status', 'Actions'],
                    data: [
                        @foreach ($certificates as $certificate)
                            [
                                '{{ $certificate->id }}',
                                '{{ $certificate->resident->last_name }}, {{ $certificate->resident->first_name }}',
                                '{{ \Carbon\Carbon::parse($certificate->date_of_death)->format('m/d/Y') }}',
                                '{{ \Carbon\Carbon::parse($certificate->date_of_issuance)->format('m/d/Y') }}',
                                `<span class="badge ${getStatusBadgeClass('{{ $certificate->status }}')}">
                                    {{ $certificate->status }}
                                </span>`,
                                `<div class="flex space-x-2">
                                    <a href="{{ route('certificate-of-death.show', $certificate->id) }}" class="btn btn-sm btn-info">View</a>
                                    <a href="{{ route('certificate-of-death.print', $certificate->id) }}" target="_blank" class="btn btn-sm btn-success">Print</a>
                                    <a href="{{ route('certificate-of-death.edit', $certificate->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                    <form action="{{ route('certificate-of-death.destroy', $certificate->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
...
function showDeathCertificateModal(certificateId) {
    ...
    fetch(`/certificate-of-death/${certificateId}`)
...
function printDeathCertificate(certificateId) {
    const printWindow = window.open(
        `/certificate-of-death/${certificateId}/print`, 
        '_blank',
