...
<body>
    <div class="page">
        ...
        <div class="content">
            <p>
                This is to certify that <strong
                    class="name">{{ strtoupper($certificate->resident->full_name) }}</strong>,
                ...
                <strong>{{ ucfirst(strtolower($barangayDetails->barangay_name ?? $certificate->resident->barangay)) }}</strong>,
            ...
            <p>
                Issued this <strong>{{ $certificate->date_of_issuance->format('jS') }}</strong> day of
        ...
        <div class="signature-row">
            <div class="applicant">
                {{-- Applicant signature area empty as per typically simple residency certs, or present? Reference has
                it. I'll include it blank or populated if available. Original residency had signature block. --}}
                <div style="margin-top:30px; padding-top:6px; text-decoration:underline">
                    {{ strtoupper($certificate->resident->full_name) }}<br></div>
    ...
    strtoupper($barangayDetails->captain_name) : '________________' }}</span>
        ...
        <div class="receipt">
            <div>Certificate No.: {{ $certificate->id ?? '__________' }}</div>
            <div>OR No.: {{ $certificate->or_number ?? '__________' }} | Amount:
                {{ $certificate->amount_paid ? '₱' . number_format($certificate->amount_paid, 2) : '__________' }}</div>
            <div>Date Issued: {{ $certificate->date_of_issuance->format('m/d/Y') }}</div>
        </div>
