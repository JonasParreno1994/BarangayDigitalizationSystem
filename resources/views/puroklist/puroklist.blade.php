    ...
    <div class="animate__animated p-6" :class="[$store.app.animation]">
        ...
        <div x-data="multipleTable">
            <div class="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary text-2xl font-bold">
                <button type="button" class="btn btn-success" @click="toggle">Add Purok</button>
                <h1 class="ltr:mr-4 rtl:ml-3 text-center w-full">List of Puroks</h1>
            </div>
            ...
            <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
                <div x-show="open" x-transition="" x-transition.duration.300="" class="panel my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0">
                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                        <div class="text-lg font-bold">Add Purok</div>
                    ...
                    <div class="p-5">
                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                            <form id="purokForm" action="{{ route('purok.store') }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="purok_name" class="form-label">Purok Name</label>
                                    <input type="text" class="form-control w-full border border-gray-300 rounded-md" id="purok_name" name="purok_name" required>
                                </div>
                                <div class="mb-3">
                                    <label for="description" class="form-label">Description</label>
                                    <textarea class="form-control w-full border border-gray-300 rounded-md" id="description" name="description" rows="3"></textarea>
                                </div>
                        </div>
        ...
        Alpine.data('multipleTable', () => ({
            ...
            init() {
                this.datatable2 = new simpleDatatables.DataTable('#myTable2', {
                    data: {
                        headings: ['ID', 'Purok Name', 'Description', 'Actions'],
                        data: [
                            @foreach ($puroks as $purok)
                                ['{{ $purok->id }}', '{{ $purok->purok_name }}', '{{ $purok->description }}', 
                                `<div class="flex space-x-2">
                                    <form action="{{ route('purok.destroy', $purok->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this purok?')">Delete</button>
                                    </form>
                                </div>`],
                            @endforeach
                        ],
