    ...
    <div class="animate__animated p-6" :class="[$store.app.animation]">
        <div x-data="multipleTable">
            <div class="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary text-2xl font-bold">
                ...
                <h1 class="ltr:mr-4 rtl:ml-3 text-center w-full">List of Good Moral Certificates</h1>
            </div>
            <div class="panel mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <form action="{{ route('barangay-good-moral.report') }}" method="GET" class="mt-4">
            ...
            <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
                <div x-show="open" x-transition x-transition.duration.300 class="panel my-8 w-full max-w-4xl overflow-hidden rounded-lg border-0 p-0">
                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                        <div class="text-lg font-bold">ISSUE GOOD MORAL CERTIFICATE</div>
                    ...
                    <div class="p-5">
                       <form id="certificateForm" action="{{ route('barangaygoodmoral.store') }}" method="POST">
                            @csrf
                        ...
                        @foreach ($certificates as $certificate)
                            [
                                ...
                                `<div class="flex space-x-2">
                                    <a href="{{ route('barangaygoodmoral.show', $certificate->id) }}" class="btn btn-sm btn-info">View</a>
                                    <a href="{{ route('barangaygoodmoral.print', $certificate->id) }}" target="_blank" class="btn btn-sm btn-success">Print</a>
                                    <a href="{{ route('barangaygoodmoral.edit', $certificate->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                    <form action="{{ route('barangaygoodmoral.destroy', $certificate->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
...
function showCertificateModal(certificateId) {
    ...
    fetch(`/barangaygoodmoral/${certificateId}`)
...
function printCertificate(certificateId) {
    const printWindow = window.open(
        `/barangaygoodmoral/${certificateId}/print`, 
        '_blank',
